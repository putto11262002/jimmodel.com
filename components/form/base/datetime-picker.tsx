...
export default function DatetimePicker({
  ...
  const handleInternalValueChange = (newValue: Date | undefined) => {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          {internalValue ? formatDate(internalValue, time) : "Pick a date"}
      ...
      <PopoverContent align="start" className="w-auto space-y-4">
        <div className="grid grid-cols-2 gap-x-2 gap-y-3">
          ...
          {internalValue && (
            <div className="col-span-full">
              <TimeSelect
                value={internalValue}
                onChange={(newValue) => handleInternalValueChange(newValue)}
              />
            </div>
          )}
        </div>
        <Calendar
          ...
          onSelect={(selected) => {
            handleInternalValueChange(selected);
          }}
